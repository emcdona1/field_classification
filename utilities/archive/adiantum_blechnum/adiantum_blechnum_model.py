...
def build_model(img_size): # create model architecture and compile it
	...
	model.add(Conv2D(10, (5, 5), input_shape = (img_size, img_size, 3), kernel_regularizer=regularizers.l2(0.01))) 
	...
	model.add(Conv2D(40, (5, 5), kernel_regularizer=regularizers.l2(0.01)))
	...
	# Decrease to 500 neurons
	model.add(Dense(500, activation="linear", kernel_regularizer=regularizers.l2(0.05))) # kernel_regularizer=regularizers.l2(0.1)))
...
def train_cross_validate(n_folds, data_dir, categories, image_size, num_epochs):
	...
	for index, (train_indices, val_indices) in enumerate(skf.split(features, labels)):
		...
		df = df.append([[index+1, history.history['loss'][epochs_trained-1], history.history['acc'][epochs_trained-1], history.history['val_loss'][epochs_trained-1], history.history['val_acc'][epochs_trained-1]]])
...
if __name__ == '__main__':
	parser = argparse.ArgumentParser('build and train model')
	parser.add_argument('-d', '--directory', default='', help='Folder holding category folders')	
	parser.add_argument('-c1', '--category1', default='adiantum_train', help='Folder of class 1')
	parser.add_argument('-c2', '--category2', default='blechnum_train', help='Folder of class 2')
