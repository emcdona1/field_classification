...
def download_images_from_csv(csv_local_name, occ_file_name, download_location):
    with open(csv_local_name, encoding='utf-8') as image_csv: #using with ensures files get closed automatically at end - good for memory
        ...
        occ_df = pd.read_csv(open(occ_file_name, encoding = 'utf-8'), usecols=['id','catalogNumber'])
...
if __name__== '__main__':
    ...
    parser.add_argument('-f', '--file', default= 'dli_images/images_final.csv', help='Upload CSV with URLs in 2nd column')
